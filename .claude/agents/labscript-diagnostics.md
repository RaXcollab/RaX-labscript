...
description: "Use this agent to analyze, interpret, or debug Labscript suite logs (BLACS.log, BLACS_faulthandler.log): diagnosing errors, warning patterns, timing issues, recurrence analysis, correlating entries across runs, or increasing logging verbosity for more diagnostic detail. Use proactively after device/config changes to verify system health.\n\nExamples:\n\n- User: \"BLACS crashed during the last shot sequence, can you figure out what happened?\"\n  Assistant: \"Let me use the labscript-diagnostics agent to analyze the recent BLACS log entries.\"\n  (Launch labscript-diagnostics to examine logs/ for errors, exceptions, and faulthandler traces.)\n\n- User: \"How did the last 10 shots go? Any issues?\"\n  Assistant: \"Let me launch the labscript-diagnostics agent to review the recent log entries.\"\n  (Launch labscript-diagnostics to parse recent entries and synthesize a shot-by-shot summary.)"
model: sonnet
color: yellow
memory: project
skills:
