...
- **ENABLED switchboard**: `connection_table.py` gates each BLACS tab behind a module-level `ENABLED` dict (PrawnBlaster excepted — always on). Workflow: flip flag → recompile CT → restart BLACS. Even-DO padding is automatic (`_pad_even_digitals`); never add manual dummy DO lines. Reserved pad lines (`PARITY_PAD_LINE`) — keep free in the CT and physically unwired: `ni_6361` → `port0/line7`, `ni_6535` → `port3/line7` (DIO 31).
