...
- **NI even-DO padding is automatic** — `_pad_even_digitals` in `connection_table.py` pads an odd DO count on the reserved line (`PARITY_PAD_LINE`): `ni_6361` → `port0/line7`, `ni_6535` → `port3/line7` (DIO 31). Keep both lines free in the CT and physically unwired; never add manual dummy DOs.
