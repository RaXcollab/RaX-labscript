...
- **HF lock spec (canonical):** lock acquires when **5** consecutive in-tolerance samples land within `lock_tolerance(port)` — default `5e-6 THz = 5 MHz`, TiSa_1 (ch4) override `1e-6 THz = 1 MHz` (since 2026-07-10) — with `LOCK_TIMEOUT_S = 60`. Code is authoritative — `GUIs/HF_Locking/workers.py:20-32`. Older "2 consecutive" or flat-5-MHz notes are stale; correct everywhere on contact.
