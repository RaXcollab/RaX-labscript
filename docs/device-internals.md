...
## BLACS Fork Specifics (RaXcollab/blacs, forked from shafinulh/blacs)
...
- **Shared `parent_port` design**: `TiSa_1_Setpoint` and `TiSa_1_Value` both have `parent_port='1'`. LaserLockTab intentionally avoids the base RemoteControlTab's second `create_analog_outputs(AM_prop)` call (which would clobber `self._AO['1']` with a monitor AnalogOutput). Comment at `LaserLockDevice/blacs_tabs.py:62-64` explains.
