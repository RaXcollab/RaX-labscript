# Old analysis code, used until 2026-03-27.
...
TRACE_NAMES = ['Absorption', 'Absorption2', 'Absorption3', 'Absorption_DCprobe']
trace_data = {}
...
if 'Absorption' in trace_data and 'Absorption2' in trace_data:
    ...
    ax1.axvline(x=tYAG_1 * 1000, color='r', linestyle='--', label='YAG')
    ax1.axvspan((tYAG_1 + ENH_START) * 1000, (tYAG_1 + ENH_START + ENH_DURATION) * 1000,
                color='yellow', alpha=0.3, label='ENH window')

ax1.set_xlim([0, XLIM_MS*3.5])
...
if 'Absorption3' in trace_data:
    ...
    ax3.plot(times_3 * 1000, values_3, 'g', label='Absorption3')
    ax3.axvline(x=tYAG_1 * 1000, color='r', linestyle='--', label='YAG')

if 'Absorption_DCprobe' in trace_data:
    analog_data_dcprobe = trace_data['Absorption_DCprobe']
    times_dcprobe = analog_data_dcprobe[0].flatten() - 2e-3
    values_dcprobe = analog_data_dcprobe[1].flatten()
    ax3.plot(times_dcprobe * 1000, values_dcprobe, 'b', label='Absorption_DCprobe')

ax3.set_xlim([0, XLIM_MS*3.5])
...
try:
    ...
    ax2.axvline(x=tYAG_1 * 1000, color='r', linestyle='--', label='YAG')
    ax2.legend()
...
if tstart is not None and tend is not None:
    ...
    YAG_PULSE = 0.5e-3  # 0.5 ms, matches sequence
    yag1 = _step(t_ms, tYAG_1, YAG_PULSE)
    yag2 = _step(t_ms, tYAG_2, YAG_PULSE if DOUBLE_YAG else np.zeros_like(t_ms))
    shutter_delay = 15e-3 # From measurements
    enh  = _step(t_ms, shutter_delay+ENH_START, ENH_DURATION)
    ...
    ax4_top.set_title(f"Trigger inputs for 50ms after tstart (tend={tend_ms:.1f} ms)", fontsize=11)
