...
- **Atomic (x, y) pair (2026-08-04).** Both coords leave BLACS as ONE `laser_raster_xy` write with value `[x, y]` (target/pixel frame), never two single-axis writes: per-axis writes make the GUI pair each new coord with its cached partner, so the motors traverse `(x_new, y_old)` — a composite that can map outside travel even when both endpoints are legal. One-coord cases keep the single-axis path (a shot table with only `laser_raster_x_coord`, or a front panel carrying one coord); the missing axis is **never** filled from `front_panel_values` — that echo can be seconds stale, and the GUI pairs a single-axis move with a fresh encoder read itself. Failure policy is unchanged: courtesy warn in `program_manual` (the hook fires once, labelled `laser_raster_xy`), strict raise in `transition_to_buffered`. The GUI also accepts an optional `args['frame']` (`pixel` default, `motor` bypasses calibration); BLACS only writes target-frame coords and sends no tag. **Deploy with the GUI side** — an old GUI answers `unknown_connection` to `laser_raster_xy` (manual writes then only warn, but every both-axis shot fails), so restart the GUI and BLACS together.
