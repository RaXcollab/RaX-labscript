...
6. When the raster path is exhausted, the shot queue stops with a "Raster sequence complete" error. Queueing more shots re-arms and restarts the raster from the beginning of the path.
...
| "Raster sequence complete" stops queue | Expected at path end. Re-queue to restart the same path from the beginning, or configure a new path in the GUI first |
